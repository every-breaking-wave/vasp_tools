...
if [ "$ispin" -eq "2" ]; then
    ...
    e1_up=$(grep "     $homo     " $outcar | head -$nkpt | sort -n -k 2 | tail -1 | awk '{print $2}')
    e2_up=$(grep "     $lumo     " $outcar | head -$nkpt | sort -n -k 2 | head -1 | awk '{print $2}')
    
    e1_down=$(grep "     $homo     " $outcar | head -$nkpt | sort -n -k 2 | tail -1 | awk '{print $3}')
    e2_down=$(grep "     $lumo     " $outcar | head -$nkpt | sort -n -k 2 | head -1 | awk '{print $3}')
